...
session_start();
    if(!isset($_SESSION['x']))
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $conn->prepare("SELECT c_id FROM complaint WHERE c_id = ?");
        ...
        if ($stmt->num_rows > 0) {
            ...
            // Show JS alert if not found
            echo "<script>alert('Complaint ID not registered.'); window.location.href='headHome.php';</script>";
            exit();
        }
       $smt->close();
    }
